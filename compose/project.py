    ...
    def events(self, service_names=None):
        ...
        def build_container_event(event):
            ...
            return {
                ...
                'action': event['status'],
                'id': event['Actor']['ID'],
                'service': container_attrs[LABEL_SERVICE],
                'attributes': dict([
        ...
        service_names = set(service_names) if service_names else None
        for event in self.client.events(
            filters={'label': self.labels()},
            decode=True
        ):
            ...
            try:
                if service_names and event['Actor']['Attributes'][LABEL_SERVICE] not in service_names:
                    continue
            except KeyError:
                pass
